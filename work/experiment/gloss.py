...
class GlossGame(object):
	def __init__(self, name):
		...
		Gloss.scaler = GL_LINEAR #GL_NEAREST
...
class Gloss(object):
	...
	def draw_lines(lines, color = Color.WHITE, width = 1.0, join = False):
		...
		for line in lines:
			...
			glVertex2f(previous[0], previous[1])
		...
		if join:
			glVertex2f(previous[0], previous[1])
			glVertex2f(lines[0][0], lines[0][1])
...
class Texture(object):
	def __init__(self, path):
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, Gloss.scaler)
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, Gloss.scaler)
...
class RenderTarget:
	def __init__(self, width = 512, height = 512):
		...
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, Gloss.scaler)
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, Gloss.scaler)
